...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.view(b, n_head, head_dim, h * w)
        k = k.view(b, n_head, head_dim, h * w)
        v = v.view(b, n_head, head_dim, h * w)

        # scaled dot-product attention (stable)
        attn = torch.einsum('bnhd,bnhe->bnhe', q, k)
        attn = attn / math.sqrt(head_dim)

        attn = torch.softmax(attn, dim=-1)

        out = torch.einsum('bnhe,bnhe->bnhd', attn, v)
